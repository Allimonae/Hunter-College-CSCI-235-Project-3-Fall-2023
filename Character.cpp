...
Georgina Woo
Jun 14 2023
Modified by Allison Lee on September 24 2023
...
@return     : Returns true if the right hand side character is "equal", false otherwise.                      Two characters are equal if they have the same name, same race, same level                      and are either both an enemy or not.
...
bool Character::operator==(const Character& right) const{
    if ((getName() == right.getName()) && (getRace() == right.getRace()) && 
        (getLevel() == right.getLevel()) && (isEnemy() == right.isEnemy())){
            return true;
        }
    else {return false;}
}
...
void Character::display() const{
    std::string enemy = "They are not";
    if (isEnemy()){enemy = "They are";}
    std::cout << getName() << " is a Level " << getLevel() << " " << getRace() << 
